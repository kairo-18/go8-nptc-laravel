...
import { Driver, Operator, Vehicle, VRCompany } from '@/lib/types';
...
export default function Pending() {
    ...
    const updatedData = data.map((row) => ({
        ...row,
        Filter: `${row.Status ? `${row.Status} ` : ''}${row.NPTC_ID || ''}`,
    }));
    ...
    return (
        <MainLayout breadcrumbs={breadcrumbs}>
            <Head title="Pending" />
            <motion.div initial="initial" animate="animate" variants={pageVariants} className="p-2">
                ...
                <AnimatePresence>
                    {selectedItem && (
                        <motion.div
                            className="relative rounded-md border bg-white p-4 shadow-md"
                        ...
                        >
                            ...
                            {selectedItem.type === 'Operator' ? (
                                ...
                                <PendingDriverDetails item={selectedItem} role={userRole} />
                ...
                <motion.div variants={cardItemVariants} className="mt-4">
